...
Show.prototype.changeMangician = function(magician) {
...
Show.prototype.loop = function(now) {
    ...
    console.log('loop', 'status:', this.showStatus, 'time:', now);
    ...
    if (this.showStatus === Show.SHOW_WAITING) {
        ...
        if (!this.magician) {
            ...
            if (!magician) {
                return console.error("Magician not found!");
            }

            return this.changeMangician(magician);
        }

        var magician = this.magician;
        magician.loop();
    } else {
        if (now.getTime() < this.end.getTime()) {
            return;
