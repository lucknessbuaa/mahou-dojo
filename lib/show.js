...
Magician.prototype.values = function() {
    var result = _.pick(this, ['first', 'id', 'name', 'avatar', 'status', 'start', 'score', 'end']);
    ['start', 'score', 'end'].forEach(function(key) {
        result[key] = result[key].getTime();
    });

    if (this.status === Magician.STATUS_FINISHED) {
        result.scores = this.scores.slice(0, 2);
    }
...
Magician.prototype.onFinish = function() {
    this.status = Magician.STATUS_FINISHED;
    console.log('My magic is finished');
...
function Show(round, now) {
    ...
    this.end = round.end;
    this.magicians = _.map(round.magicians, function(data) {
